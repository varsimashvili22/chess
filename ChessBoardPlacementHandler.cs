    ...
    internal void ClearHighlights() {
        for (var i = 0; i < 8; i++) {
            for (var j = 0; j < 8; j++) {
                ...
                if (tile.transform.childCount <= 0 || tile == null) continue;
                foreach (Transform childTransform in tile.transform) {
                    if(childTransform.GetComponent<ChessPlayerPlacementHandler>() == null) //pieces are children of tiles so we dont want them destroyed
                    {
